...
def process(transcript_relative):
    ...
    df['file_path'] = df['file_path'].apply(lambda x: transcript.replace('transcription_n2w.txt','audio/')+x.split('-')[1]+'/'+x.replace('.m4a','.wav'))
    ...
    df.to_csv(transcript.replace('transcription_n2w.txt','bucket.csv'),index=False)
    # print(transcript)
    print("Done",transcript,'\n')


path = sys.argv[1] # 'kb_data_clean_wav'
all_transcripts = glob.glob(f'{path}/**/transcription_n2w.txt',recursive=True)
Parallel(n_jobs=-120)(
    delayed(process)(t) for t in tqdm.tqdm(all_transcripts)
)

csv_list = glob.glob(f"{path}/**/*.csv",recursive=True)
...
if 'clean' in path:
    # copy valid to train
    valids = glob.glob(f"{path}/**/valid/**/valid.*",recursive=True)
    for v in valids:
