...
class DataProcessor:
    def __init__(
        ...
        max_prompt_length: int = 448, #448,  #223
        max_tokens_length: int = 440, #440   #219
        subsampling_factor_for_silence: int = 1,
        tokenizer_type: str = "multilingual",
        normalize_unicode: bool = False,
        indicnormalizer : bool = True,
        prompt: str = '',
        split: str = '',
        HFDataset: str = 'hi_in'
    ) -> None:
        ...
        self.indicnormalizer = indicnormalizer
        self.prompt = prompt
        self.split = split
    ...
    def _verify_args(self) -> None:
        if self.with_timestamps:
            ...
            if not self.data_file and not self.HFDataset:
    ...
    def _process_without_timestamps(self) -> None:
        ...
        if self.HFDataset is not None:        
            try:
                from datasets import load_dataset
            except:
                print('Install pip install datasets')

            dataset = load_dataset("google/fleurs",self.HFDataset,split=self.split,cache_dir='dataset/HF')
            ...
            audio_paths = df['file_path']
            transcriptions = df['transcript']

        count = 0
        for i in range(len(audio_paths)):
            audio_path, text = audio_paths[i],transcriptions[i]
            if self.HFDataset is not None:
                ind=audio_path.rfind('/')
                if self.split == 'validation':
                    audio_path = audio_path[:ind]+f'/dev'+audio_path[ind:]
                else:                      
                    audio_path = audio_path[:ind]+f'/{self.split}'+audio_path[ind:]
            if self.indicnormalizer:
                normalizer = IndicTextNormalizer(use_indic_normalizer = True, lang = self.language)
            ...
            record = Record(audio_path=audio_path, text=text, language=self.language, prompt = self.prompt)
...
def main():
    ...
    processor = DataProcessor(
        ...
        indicnormalizer = args.indicnormalizer,
        prompt = args.prompt,
        split = args.split,
    ...
    # data_file = "dataset/kathbath/kb_data_clean_wav/"
    # split = ['train','validation']
    # output = "preprocess_data/kathbath/"
    # for i in range(8):
    #     cur_lang = language[i]
    #     #split wise
    #     for j in range(2):
    #         cur_split = split[j]
    #         cur_data_file = data_file + language[i] + '/' + split[j] + '/bucket.csv'
    #         cur_output = output + language[i] + '/' + 'train' + '.json'
            
    #         if split[j]=='validation':
    #             cur_data_file = data_file + language[i] + '/' + 'valid' + '/bucket.csv'
    #             cur_output = output + language[i] + '/' + 'dev' + '.json'
    ...
    #         print('Lang: '+ cur_lang + ' , split: ' + cur_split +' completed.')
